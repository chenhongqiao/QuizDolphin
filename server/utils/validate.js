class DataValidate {
  ...
  static validateQuizData(email, questions, answers, duration, attemptId, quizId) {
    // Logic check on questions and answers are not performed
    // since QuizData are generated by backend.
    if (typeof email !== 'string' || !Array.isArray(questions) || !Array.isArray(answers)
    || typeof duration !== 'number' || typeof attemptId !== 'string' || typeof quizId !== 'string') {
      return false;
    }
    return true;
  }
  ...
  static validateQuizResult(score, questions, results, totalPoints, attemptId, email, quizId) {
    if (typeof score !== 'number' || !Array.isArray(questions)
    || !Array.isArray(results) || typeof totalPoints !== 'number' || typeof attemptId !== 'string'
    || typeof email !== 'string' || typeof quizId !== 'string') {
      return false;
    }
    return true;
  }
